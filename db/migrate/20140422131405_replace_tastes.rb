...
class ReplaceTastes < ActiveRecord::Migration
  def up
    new =
    ...
    puts 'ALL NEW CODES: ' + codes.join(',')

    to_remove = Taste.where('code NOT IN(?)', codes)
    puts 'REMOVE:' + to_remove.map { |taste| taste.code }.join(', ')
    to_remove.delete_all

    codes.each_with_index do |code, i|
      attrs = { code: code, name: new[i][0], kind: (new[i][2] || 0) }
      puts attrs
